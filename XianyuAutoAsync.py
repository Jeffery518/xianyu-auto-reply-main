...
class XianyuLive:
    ...
    # 类级别变量，记录每个账号最后一次成功密码登录的时间，并在多实例间共享（简化处理）
    _last_password_login_time: Dict[str, float] = {}
    _password_login_cooldown = 3600  # 密码登录冷却时间(秒)，即1小时内最多只允许一次密码登录尝试
        ...
        try:
            ...
            try:
                ...
                slider_stealth = XianyuSliderStealth(
                    ...
                    headless=True  # 🔧 恢复无头模式以节省内存，已保留轨迹优化逻辑
                )
    ...
    async def _try_password_login_refresh(self, trigger_reason: str = "令牌/Session过期"):
        ...
        try:
            ...
            # 提前记录密码登录时间，防止登录失败导致的无限重试循环
            XianyuLive._last_password_login_time[self.cookie_id] = time.time()
            logger.warning(f"【{self.cookie_id}】已记录密码登录尝试时间，冷却期 {XianyuLive._password_login_cooldown} 秒")
            
            # 使用 wait_for 防止 Playwright 内部死锁导致线程永久阻塞
            try:
                ...
                logger.error(f"【{self.cookie_id}】密码登录刷新执行线程时发生异常: {self._safe_str(e)}")
                return False
        ...
        try:
            ...
            while True:
                try:
                    ...
                    # 修复: 某些Python环境(如3.14)或者未将当前线程绑定默认事件循环的情况下，直接调 traceback.format_exc() 会间接触发 asyncio.get_event_loop() 抛出 RuntimeError。
                    # 这里改用安全的方式获取堆栈
                    try:
                        stack_str = "".join(traceback.format_exception(type(e), e, e.__traceback__))
                        logger.warning(f"【{self.cookie_id}】异常堆栈:\n{stack_str}")
                    except Exception as inner_e:
                        logger.warning(f"【{self.cookie_id}】无法完整获取异常堆栈: {inner_e}")
